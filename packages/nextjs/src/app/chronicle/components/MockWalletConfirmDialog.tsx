...
const MockWalletConfirmDialog = ({
  ...
  return (
    ...
    >
      <div
        className="sds-panel relative w-full max-w-3xl overflow-hidden rounded-[2rem] border border-[#f0642f]/20 bg-[linear-gradient(145deg,rgba(12,13,15,0.98),rgba(18,20,22,0.94))] px-6 py-6"
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-7 text-[#f4efe2]/68">
        ...
        {receipt ? (
          <>
            <div className="mt-6 rounded-[1.2rem] border border-[#f0642f]/18 bg-[linear-gradient(180deg,rgba(240,100,47,0.08),rgba(255,255,255,0.03))] px-4 py-4">
              ...
              <div className="mt-4 grid gap-3 text-xs leading-6 text-[#f4efe2]/68 sm:grid-cols-2">
                ...
                <div>
                  ...
                  {truncateMiddle(receipt.registryObjectId, 10, 8)}
                </div>
                <div>
                  <span className="font-mono uppercase tracking-[0.16em] text-white/42">
                    {t('fields.template')}
                  </span>{' '}
                  {truncateMiddle(receipt.templateObjectId, 10, 8)}
                </div>
                <div>
                  <span className="font-mono uppercase tracking-[0.16em] text-white/42">
                    {t('fields.gasPayment')}
                  </span>{' '}
                  {truncateMiddle(receipt.gasPaymentObjectId, 10, 8)}
                </div>
                <div>
                  <span className="font-mono uppercase tracking-[0.16em] text-white/42">
                    {t('fields.gasBudget')}
            ...
            <div className="mt-6 rounded-[1.2rem] border border-white/10 bg-black/18 px-4 py-4">
              <div className="font-mono text-[0.62rem] uppercase tracking-[0.3em] text-[#ffd2c2]">
                {t('callTitle')}
              </div>
              ...
              <div className="mt-6 rounded-[1.2rem] border border-white/10 bg-black/18 px-4 py-4">
                ...
                <div className="mt-4 grid gap-3 text-xs leading-6 text-[#f4efe2]/68 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    {new Date(
                      Number(receipt.claimPayload.issuedAtMs)
                    ).toLocaleString()}
                  </div>
                  <div>
                    <span className="font-mono uppercase tracking-[0.16em] text-white/42">
                    ...
                    {new Date(
                      Number(receipt.claimPayload.deadlineMs)
                    ).toLocaleString()}
                  </div>
                  <div className="sm:col-span-2">
                    ...
                    <span className="break-all">
                      {receipt.claimPayload.evidenceUri}
                    </span>
                  </div>
                  <div>
                    <span className="font-mono uppercase tracking-[0.16em] text-white/42">
                    ...
                    {truncateMiddle(
                      receipt.claimPayload.signerPublicKey,
                      14,
                      10
                    )}
                  </div>
