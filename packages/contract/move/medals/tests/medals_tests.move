...
fun test_claim_medal_with_test_ticket() {
    ...
    let mut chain_clock: clock::Clock = ts.take_shared();
    chain_clock.set_for_testing(1_000);
    ...
    assert!(medals::awarded_at_ms(&medal) == 1000, 6);
...
fun test_admin_mint_all_eight_medals() {
    ...
    while (i < medal_kinds.length()) {
        ...
        medals::admin_mint(&admin_cap, &mut registry, &template, owner, ts.ctx());
...
#[test, expected_failure(abort_code = 5, location = medals)]
fun test_claim_medal_rejects_expired_ticket() {
    let owner = @0xA;
    let mut ts = ts::begin(owner);
    ...
    let template: medals::MedalTemplate = ts.take_shared_by_id(template_id);
    let mut chain_clock: clock::Clock = ts.take_shared();
    chain_clock.set_for_testing(2_001);
    medals::claim_medal_with_test_ticket(
        &mut registry,
        &template,
        &chain_clock,
...
#[test, expected_failure(abort_code = 1, location = medals)]
fun test_admin_mint_cannot_duplicate_same_medal() {
    let owner = @0xA;
    let mut ts = ts::begin(owner);
