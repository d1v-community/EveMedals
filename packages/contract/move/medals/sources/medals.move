...
const EInvalidClaimWindow: u64 = 17;
...
public fun claim_medal(
    ...
    assert_medal_not_claimed(registry, owner, template.medal_kind);
    ...
    assert!(issued_at_ms <= deadline_ms, EInvalidClaimWindow);
    assert!(issued_at_ms <= now_ms, EInvalidClaimWindow);
...
public fun admin_mint(
    ...
    assert!(template.active, ETemplateInactive);
    assert_medal_not_claimed(registry, recipient, template.medal_kind);
    mark_medal_claimed(registry, recipient, template.medal_kind);
...
fun mint_medal(
    ...
    mark_medal_claimed(registry, owner, template.medal_kind);
...
fun claim_key(owner: address, medal_kind: u8): ClaimKey {
    ClaimKey { owner, medal_kind }
}

fun assert_medal_not_claimed(registry: &MedalRegistry, owner: address, medal_kind: u8) {
    assert!(
        !field::exists_(&registry.id, claim_key(owner, medal_kind)),
        EMedalAlreadyClaimed,
    );
}

fun mark_medal_claimed(registry: &mut MedalRegistry, owner: address, medal_kind: u8) {
    field::add(&mut registry.id, claim_key(owner, medal_kind), true);
}
