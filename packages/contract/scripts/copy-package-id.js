...
/**
 * The script copies the deployed package ID from the corresponding Suibase network file to .env.local of the Next.js package,
...
const main = async () => {
  ...
  // Check whether the Next.js package exists to decide what environment variable name format to use.
...
const targetFilePath = () => {
  return path.join(process.cwd(), "../nextjs/.env.local");
};
...
/**
 * Check if next.config.ts exists in the Next.js package.
 *
 * @returns {boolean}
 */
const isNextJsProject = async () => {
  try {
    await promises.stat(path.join(process.cwd(), "../nextjs/next.config.ts"));
    return true;
  } catch {
    return false;
  }
